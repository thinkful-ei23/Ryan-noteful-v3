...
router.get('/', (req, res, next) => {
  ...
  }//if search term isnt null/undefined, continue to search
  ...
  Note.find(filter)
    .populate('tags')//return note With tags
    .sort({ updatedAt: 'desc' })
    .then(results => {
      res.json(results);
    })
...
router.put('/:id', (req, res, next) => {
  ...
  Promise.all([
    ...
    .then(() => {
      return Note.findByIdAndUpdate(id, updateNote, { new: true })//new: true ensures returning the updated obj
        .populate('tags');
    })
    .then(result => {
      if (result) {
